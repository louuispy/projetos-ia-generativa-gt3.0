...
**Passo a passo em vídeos de como construir o projeto:**

https://youtu.be/-eNGxNVulMo?si=EZ0K2eLUN7WWYi45

https://youtu.be/NC9tJ8yA3E8?si=_esWsttVKABTeylu

https://youtu.be/7ylil5QMA_I?si=kGDjrMkTq8wKUgJ8

https://youtu.be/YZDikzZQ69M?si=m_YDTYXWfaXm0b1W

https://youtu.be/WG8tKc8kZwM?si=hENh-uvaqzEXt0lj

https://youtu.be/3ZQgq6Quitg?si=7jzowiOc67GCVBJd

https://youtu.be/Yg9c43skuRQ?si=pkmD0huWax67KyVV

https://youtu.be/pgNTrrqTJ1Q?si=wJWBNXzn45dQjau5
...
Esse projeto vai ser relativamente diferente do que fizemos antes, pois vai ser construído de uma forma mais estruturada, com mais arquivos, e utilizando o sistema de embeddings que vimos na aula mais cedo.
...
O fluxo dessa função para fazer o LLM enxergar imagens será o seguinte:

- Usuário envia imagem
- Imagem é codificada para base64
- Imagem codifiada é enviada pro LLM
- LLM recebe a imagem, decodifica e lê
- LLM gera uma descrição detalhada da imagem
- LLM retorna a descrição da imagem pro usuário
...
Estamos criando uma `Human Message`, ou seja, estamos definindo para o LLM o que seria a mensagem do usuário, que será enviada.

O conteúdo (content) da mensagem será diferente.

Enquanto nas outras vezes passamos diretamente a pergunta do usuário entre aspas, aqui iremos passar o content na estrutura multimodal do LangChain, ou seja, vamos criar uma HumanMessage onde o seu conteúdo pode ser texto, imagem, áudios…

No nosso caso, vamos enviar uma imagem.

Com isso, a estrutura do nosso HumanMessage será dividida em dois blocos:
...
`"type": "text"`: indica que esse conteúdo a seguir, desse bloco, será um texto.

`"text: "Descreva detalhadamente o que há nesta imagem"` : é a mensagem que estamos passando pro LLM. O prompt do usuário. O que queremos que o LLM faça.
...
Aqui estamos enviando a imagem pro LLM.

`"type": "image_url"`: indica que o conteúdo desse bloco será uma imagem. Essa imagem pode ser enviada tanto como uma URL como também em texto base64. Por conta disso, passamos que o tipo da mensagem é `image_url`, porque nesse caso de leitura de imagens, isso é algo **exigido** como tipo.
...
A estrutura é na seguinte ordem:  `data:tipo_da_imagem/base64,texto_base64_da_imagem`

Nós estamos criando uma URL da imagem que queremos enviar.

Passamos como primeiro parâmetro o tipo da imagem, se é **png, jpeg, webp**… No nosso caso, especificamos que nossas imagens enviadas serão em PNG.

Em seguida, indicamos que nossa imagem está em **base64** e enviamos o texto da imagem.
...
> **DESAFIO:** 
*Diante desse problema, onde só são permitidas imagens em PNG, o que você faria para que fosse detectado automaticamente o formato da imagem (.png, .jpeg e etc) e com isso, a imagem fosse enviada com seu respectivo formato de forma automática?*
> 
</aside>
